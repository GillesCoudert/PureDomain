...
- Toutes les librairies de ce framework se basent sur PureTrace, proposant une approche fonctionnelle pour la gestion des erreurs et des résultats :
    - Les fonctions retournent un `Result` ou `ResultAsync` au lieu de lancer des exceptions ou de retourner des Promises.
    - Les erreurs sont catégorisées en `processError` (erreurs corrigeables par le demandeur) et `technicalIssue` (erreurs non corrigeables).
    - Les codes d'erreur sont en camelCase et reflètent l'erreur (ex: `invalidCredentials` pour une erreur d'identifiants invalides).
    - Les fonctions utilisent les fonctions `generateFailure` pour créer des erreurs, jamais `new Failure()`, sauf si plusieurs erreurs (via generateError) doivent être associées à la Failure.
    - Les fonctions fluides doivent être utilisées pour propager les erreurs et les traces à travers les appels de fonctions.
